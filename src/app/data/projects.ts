...
export const projects: Record<string, Project> = {
  ...
  electronicSeal: {
    ...
    summary: "Spring Framework(WAR)와 Spring Boot(JAR) 이중 환경에서 공통 비즈니스 로직을 Maven 멀티 모듈로 통합 관리하고, React→JSP 전환 및 멀티 DB 지원 구조를 설계한 프로젝트",
    role: "Fullstack",
    tech: ["Spring Boot", "Spring Framework", "Maven", "JSP", "Java", "MariaDB", "Oracle"],
    keyContributions: [
      "Maven 멀티 모듈 구조 설계(Root→common→boot/legacy) — Controller, Config, Service를 common에 배치하여 양쪽 환경에서 동일 코드 자동 스캔",
      "Spring Boot(.yml) / Spring Framework(.properties) 설정 이중화 — 동일한 키(spring.datasource.*)로 호환성 확보",
      "React(MUI) 기반 18개 페이지를 JSP 환경으로 전환 착수 — InternalResourceViewResolver로 양 환경 동일 뷰 경로 사용",
      "멀티 DB 지원 — DataSourceConfig를 common 모듈에 배치, 드라이버 클래스 변경만으로 MariaDB/Oracle 전환 가능",
      "망분리(오프라인) 환경 대응 — local-maven-repo 기반 로컬 빌드 체계 구축"
    ],
    problem: "Spring Framework(외장 Tomcat/WAR)와 Spring Boot(내장 Tomcat/JAR) 두 환경이 동시 존재하여 동일 기능을 중복 개발해야 하는 문제와 고객사별 DB 환경(MariaDB/Oracle) 대응이 필요했습니다.",
    solution: "Maven 멀티 모듈로 공통 비즈니스 로직을 common 모듈에 분리하고, 설정 이중화와 멀티 DB 지원 구조를 설계하여 중복 개발을 방지했습니다.",
    impact: "서로 다른 프레임워크 환경에서 공통 코드를 효율적으로 관리하는 구조 설계 경험을 쌓았습니다. Boot와 Framework의 설정 차이를 흡수하는 코드 재사용 구조와 멀티 DB 무중단 전환 체계를 확립했습니다.",
